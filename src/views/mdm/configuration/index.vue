  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
      <el-form-item label="版本" prop="variantCode">
        <el-select v-model="queryParams.variantCode" placeholder="请选择版本" clearable filterable style="width: 200px">
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['mdm:configuration:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['mdm:configuration:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['mdm:configuration:remove']"
        >删除</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" width="260" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['mdm:configuration:query']"
          >查看历史</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 选项码绑定管理对话框 -->
    <el-dialog
      :title="'选项码绑定 - ' + currentConfigurationCode"
    ...
    >
      <template #detail-fields="{ data }">
        <el-form-item label="配置代码">{{ data.code }}</el-form-item>
        <el-form-item label="配置名称">{{ data.name }}</el-form-item>
        <el-form-item label="本地化名称">{{ data.nameLocal }}</el-form-item>
        <el-form-item label="版本代码">{{ data.variantCode }}</el-form-item>
...
import {
  ...
  bindConfigurationOptionCode,
  unbindConfigurationOptionCode,
  listConfigurationOptionCodes
} from "@/api/mdm/configuration";
...
export default {
  ...
  data() {
    return {
      ...
      historyList: [],
      historyFields: [
        { prop: 'code', label: '配置代码' },
        { prop: 'name', label: '配置名称' },
        { prop: 'nameLocal', label: '本地化名称' },
        { prop: 'variantCode', label: '版本代码' },
  ...
  methods: {
    getList() {
      ...
      listConfiguration(this.queryParams).then(response => {
        this.configurationList = response.data.rows;
    ...
    getVariantOptions() {
      listVariant({ page: 1, size: 1000 }).then(response => {
        this.variantOptions = response.data.rows;
      });
    },
    ...
    handleUpdate(row) {
      ...
      getConfiguration(code).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id !== undefined) {
            updateConfiguration(this.form.code, this.form).then(response => {
    ...
    handleDeactivate(row) {
      const code = row.code;
      this.$modal.confirm('是否确认停用配置"' + code + '"？').then(function() {
        return deactivateConfiguration(code, '');
      }).then(() => {
        this.getList();
    ...
    loadHistory() {
      ...
      listConfigurationHistory(this.historyCode).then(response => {
    ...
    handleManageOptionCodes(row) {
      this.currentConfigurationCode = row.code;
    ...
    loadBindings() {
      ...
      listConfigurationOptionCodes(this.currentConfigurationCode).then(response => {
    ...
    handleBind() {
      ...
      const data = {
        configurationCode: this.currentConfigurationCode,
        optionFamilyCode: this.bindForm.optionFamilyCode,
        optionCodeCode: this.bindForm.optionCodeCode,
        operator: ''
      };
      bindConfigurationOptionCode(this.currentConfigurationCode, data).then(() => {
    ...
    handleUnbind(row) {
      this.$modal.confirm('是否确认解绑选项码"' + row.optionCodeCode + '"？').then(() => {
        return unbindConfigurationOptionCode(this.currentConfigurationCode, row.optionCodeCode, '');
      }).then(() => {
